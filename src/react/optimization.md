...
From parent compoement we just pass a `postId` to child and it is reponsibility of child to either fetch the data from API by ID or if the data is already stored then data can be extract by ID from state.
